...
class CombinationSum {
    ...
    class BacktrackingMemo {
        private data class Key(val start: Int, val rem: Int)

        fun combinationSum(candidates: IntArray, target: Int): List<List<Int>> {
            candidates.sort()
            val memo = HashMap<Key, List<List<Int>>>()

            fun solve(start: Int, rem: Int): List<List<Int>> {
                if (rem == 0) return listOf(emptyList())
                if (rem < 0) return emptyList()

                val key = Key(start, rem)
                ...
                for (i in start until candidates.size) {
                    ...
                    val tails = solve(i, rem - v) // i => unlimited reuse
                    for (t in tails) {
                        // prepend v to each tail
                        out.add(listOf(v) + t)
                    }
                }
